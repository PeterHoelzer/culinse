      ...
      <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 flex flex-col hover:-translate-y-1 hover:shadow-md transition-all duration-200">
        <Link href={`/recipe/${recipe.recipe_id}`} className="relative h-44 block">
          ...
          <div className="absolute top-3 right-3 flex gap-1.5">
            <button onClick={(e) => { e.preventDefault(); setShowCollectionModal(true); }} title="Add to collection"
            ...
            <button onClick={(e) => { e.preventDefault(); onRemove(recipe.id); }} title="Remove from saved"
        ...
        <div className="p-4 flex flex-col gap-2 flex-1">
          ...
          <div className="flex items-center gap-3 text-xs text-gray-500 mt-auto pt-2">
            ...
            <Link href={`/recipe/${recipe.recipe_id}`} className="ml-auto text-orange-500 font-medium hover:text-orange-700 transition-colors">Details →</Link>
          </div>
        </div>
      </div>
      {showCollectionModal && (
        <AddToCollectionModal
          recipe={{ id: recipe.recipe_id, title: recipe.title, image: recipe.image, source: recipe.source, sourceUrl: recipe.source_url, time: recipe.time }}
...
export default function SavedPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      setUser(data.user);
      const { data: saved } = await supabase.from("saved_recipes").select("*").eq("user_id", data.user.id).order("created_at", { ascending: false });
      setRecipes(saved || []);
      setLoading(false);
    });
  }, []);
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        ...
        {!loading && recipes.length === 0 && (
          <div className="text-center py-24">
            ...
            <Link href="/" className="inline-flex items-center gap-2 px-6 py-3 rounded-full text-white text-sm font-semibold hover:opacity-90 transition-opacity" style={{ background: "#f97316" }}>
              Discover Recipes →
            </Link>
          </div>
        )}
        {!loading && recipes.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {recipes.map((recipe, i) => <SavedCard key={recipe.id} recipe={recipe} index={i} onRemove={handleRemove} />)}
          </div>
        )}
      </main>
    </div>
