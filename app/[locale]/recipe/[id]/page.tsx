...
async function fetchRecipe(id: string): Promise<Recipe | "missing" | null> {
  try {
    // v=4 (17.09.26, Render-API contain-Fix): Cache-Buster — der Vercel Data Cache ueberlebt Deploys (bis zu 24 h).
...
async function fetchSimilar(id: string, locale: string, recipe: Recipe): Promise<SimilarRecipe[]> {
  const params = new URLSearchParams({
    id,
    lang: locale === "de" ? "de" : locale === "es" ? "es" : locale === "fr" ? "fr" : locale === "it" ? "it" : locale === "pl" ? "pl" : locale === "tr" ? "tr" : locale === "nl" ? "nl" : locale === "cs" ? "cs" : "en",
    number: "8",
  });
